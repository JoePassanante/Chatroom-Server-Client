...
public class ServerMulti {
	...
	/**
	 * 
	 * @param line sends out a string to all current clients
	 * used by clients to forward their messages to other people in the server
	 * @param l 
	 */
	protected void forwardMessage(String line, long threadID) {
		//System.out.println("ServerGOT: "+line);
		for(ClientThread c: _clients){
			if(threadID != c.getId()){
				c.sendMessage(line);
			}
